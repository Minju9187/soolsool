    ...
    <div className="px-4 py-8 animate-in slide-in-from-bottom-8 duration-500">
      ...
      <form className="space-y-8 pb-10" onSubmit={(e) => e.preventDefault()}>
        ...
        <section className="flex flex-col items-center justify-center w-full h-40 bg-slate-100 dark:bg-zinc-800/50 rounded-3xl border-2 border-dashed border-slate-300 dark:border-zinc-700 cursor-pointer hover:bg-slate-200 dark:hover:bg-zinc-800 transition-colors">
          ...
          <span className="text-sm font-medium text-slate-500">사진 추가하기</span>
        </section>
        ...
        <section className="space-y-5 bg-white dark:bg-zinc-900/50 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-zinc-800">
          <div className="space-y-2">
            ...
            <Input placeholder="술 이름을 입력하세요" className="h-12 border-0 bg-slate-50 dark:bg-zinc-800 font-semibold text-lg px-4 rounded-xl" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input type="date" defaultValue={new Date().toISOString().split('T')[0]} className="h-12 border-0 bg-slate-50 dark:bg-zinc-800 px-4 rounded-xl text-slate-700 dark:text-slate-200 font-medium" />
          </div>
        </section>
        ...
        <Button className="w-full h-14 rounded-2xl bg-orange-600 hover:bg-orange-700 text-white font-bold text-lg shadow-xl shadow-orange-600/20 active:scale-[0.98] transition-all">
          저장하기
        </Button>
      </form>
    </div>
